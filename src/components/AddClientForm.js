...
const AddClientForm = ({ show, onHide, onAddClient }) => {
  ...
  const calculateDetails = () => {
    ...
    if (selectedPackage === "1 month") {
      ...
      revisionDates.push(
        new Date(enrollDate.getTime() + 15 * 24 * 60 * 60 * 1000)
      ); // add 15 days for 2nd revision
      endDate.setMonth(endDate.getMonth() + 1); // End date after 1 month
    } else if (selectedPackage === "2 months") {
      numRevisions = 4; // 1 initial and 1 every 15 days
      revisionDates = [enrollDate];
      for (let i = 1; i < numRevisions; i++) {
        revisionDates.push(
          new Date(enrollDate.getTime() + i * 15 * 24 * 60 * 60 * 1000)
        ); // add 15 days interval for each revision
      }
      endDate.setMonth(endDate.getMonth() + 2); // End date after 2 months
    ...
    const endFormatted = `${String(endDate.getDate()).padStart(
      2,
      "0"
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const newClient = calculateDetails();
